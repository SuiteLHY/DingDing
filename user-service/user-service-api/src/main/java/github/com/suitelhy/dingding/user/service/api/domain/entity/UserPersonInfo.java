 ...
 * [用户 -> 个人信息]
 *
 * <p>关于 id 生成策略, 个人倾向于使用"代理键" ———— 所选策略还是应该交由数据库来实现.</p>
 *
 * @Reference
 * {@linkplain <a href="https://dzone.com/articles/persisting-natural-key-entities-with-spring-data-j">Persisting Natural Key Entities With Spring Data JPA</a> Persisting Natural Key Entities With Spring Data JPA}
 */
@Entity
...
//    }
    ...
    /**
     * {@linkplain UserPersonInfo [用户 -> 个人信息]} 在数据层面上的等效比较
     *
     * @Description 所有字段数据要求一致.
     *
     * @param thatUserPersonInfo
     *
     * @return {@linkplain Boolean#TYPE 判断结果}
     */
    public boolean equalsOnAllData(@NotNull UserPersonInfo thatUserPersonInfo) {
        if (null == thatUserPersonInfo || ! thatUserPersonInfo.isEntityLegal()) {
            return false;
        }
        return this.isEntityLegal()
                && this.id.equals(thatUserPersonInfo.id)
                && this.username.equals(thatUserPersonInfo.username)
                && this.nickname.equals(thatUserPersonInfo.nickname)
                && this.age.equals(thatUserPersonInfo.age)
                && this.faceImage.equals(thatUserPersonInfo.faceImage)
                && this.introduction.equals(thatUserPersonInfo.introduction)
                && this.sex.equals(thatUserPersonInfo.sex)
                && this.dataTime.equals(thatUserPersonInfo.dataTime);
    }
    ...
    public enum Factory
         ...
         * @throws IllegalArgumentException
         */
        public UserPersonInfo update(@NotNull String id
                , @NotNull String username
                , @NotNull String nickname
                , @Nullable Integer age
                , @Nullable String faceImage
                ...
                throws IllegalArgumentException
        {
            ...
            return new UserPersonInfo(id, username, nickname
                    , age, faceImage, introduction
                    , sex);
        }
    ...
    private boolean setNickname(@NotNull String nickname) {
        if (Validator.USER.nickname(nickname)) {
     ...
     */
    public boolean setNickname(@NotNull String nickname, @NotNull SecurityUser operator, @NotNull SecurityUserRole operator_userRole
            , @NotNull SecurityRole operator_role)
    {
        ...
        if (Validator.USER.nickname(nickname)
                ...
                && (null != operator_role && ! operator_role.isEmpty() && null != (operator_roleVo = SecurityRole.changeIntoRoleVo(operator_role)))
                && (operator_roleVo.isAdministrator() || this.equals(operator)))
        {
    ...
    private boolean setAge(Integer age) {
        if (Validator.USER.age(age)) {
            this.age = age;
            return true;
        }
     ...
     * @return {@linkplain Boolean#TYPE 操作结果}
     */
    public boolean setAge(@NotNull Integer age, @NotNull SecurityUser operator, @NotNull SecurityUserRole operator_userRole
            , @NotNull SecurityRole operator_role)
    {
        Security.RoleVo operator_roleVo;
        if (Validator.USER.age(age)
    ...
    private boolean setFaceImage(String faceImage) {
        if (Validator.USER.faceImage(faceImage)) {
     ...
     * @return {@linkplain Boolean#TYPE 操作结果}
     */
    public boolean setFaceImage(@NotNull String faceImage, @NotNull SecurityUser operator, @NotNull SecurityUserRole operator_userRole
            , @NotNull SecurityRole operator_role)
    {
        Security.RoleVo operator_roleVo;
        if (Validator.USER.faceImage(faceImage)
                ...
                && (operator_roleVo.isAdministrator() || this.equals(operator)))
        {
            return this.setFaceImage(faceImage);
        }
        return false;
    }
    ...
    private boolean setIntroduction(String introduction) {
        if (Validator.USER.introduction(introduction)) {
     ...
     * @return {@linkplain Boolean#TYPE 操作结果}
     */
    public boolean setIntroduction(@NotNull String introduction, @NotNull SecurityUser operator, @NotNull SecurityUserRole operator_userRole
            , @NotNull SecurityRole operator_role)
    {
        Security.RoleVo operator_roleVo;
        if (Validator.USER.introduction(introduction)
                ...
                && (operator_roleVo.isAdministrator() || this.equals(operator)))
        {
            return this.setIntroduction(introduction);
        }
        return false;
    }

    public Human.SexVo getSex() {
        return Toolbox.VoUtil.getInstance()
                .getVo(Human.SexVo.class, sex);
    }

    private boolean setSex(Human.SexVo sexVo) {
     ...
     * @return {@linkplain Boolean#TYPE 操作结果}
     */
    public boolean setSex(@NotNull Human.SexVo sexVo, @NotNull SecurityUser operator, @NotNull SecurityUserRole operator_userRole
            , @NotNull SecurityRole operator_role)
    {
        Security.RoleVo operator_roleVo;
        if (Validator.USER.sex(sexVo)
