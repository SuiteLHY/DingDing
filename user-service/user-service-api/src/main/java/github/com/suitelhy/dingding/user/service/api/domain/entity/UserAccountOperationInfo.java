...
//    }
    ...
    /**
     * {@linkplain UserAccountOperationInfo [用户 -> 账户操作基础记录]} 在数据层面上的等效比较
     *
     * @Description 所有字段数据要求一致.
     *
     * @param thatUserAccountOperationInfo
     *
     * @return {@linkplain Boolean#TYPE 判断结果}
     */
    public boolean equalsOnAllData(@NotNull UserAccountOperationInfo thatUserAccountOperationInfo) {
        if (null == thatUserAccountOperationInfo || ! thatUserAccountOperationInfo.isEntityLegal()) {
            return false;
        }
        return this.isEntityLegal()
                && this.id.equals(thatUserAccountOperationInfo.id)
                && this.username.equals(thatUserAccountOperationInfo.username)
                && this.ip.equals(thatUserAccountOperationInfo.ip)
                && this.lastLoginTime.equals(thatUserAccountOperationInfo.lastLoginTime)
                && this.registrationTime.equals(thatUserAccountOperationInfo.registrationTime)
                && this.dataTime.equals(thatUserAccountOperationInfo.dataTime);
    }
    ...
    public enum Validator
        ...
        public boolean ip(@NotNull String ip) {
            return NetUtil.getInstance().validateIpAddress(ip)
                    && ip.length() < 51;
        }
    ...
    public enum Factory
         ...
         */
        @Override
        public @NotNull UserAccountOperationInfo createDefault() {
     ...
     */
    public boolean setIp(@NotNull String ip, @NotNull SecurityUser operator, @NotNull SecurityUserRole operator_userRole
            , @NotNull SecurityRole operator_role)
    {
        ...
        if (Validator.USER.ip(ip)
                ...
                && operator_roleVo.isAdministrator())
        {
    ...
    private boolean setLastLoginTime(@NotNull String lastLoginTime) {
        if (Validator.USER.lastLoginTime(lastLoginTime)) {
     ...
     * @return {@linkplain Boolean#TYPE 操作结果}
     */
    public boolean setLastLoginTime(@NotNull String lastLoginTime, @NotNull SecurityUser operator, @NotNull SecurityUserRole operator_userRole
            , @NotNull SecurityRole operator_role)
    {
        Security.RoleVo operator_roleVo;
        if (Validator.USER.lastLoginTime(lastLoginTime)
                ...
                && operator_roleVo.isAdministrator())
        {
            return this.setLastLoginTime(lastLoginTime);
        }
        return false;
    }
    ...
    private boolean setRegistrationTime(@NotNull String registrationTime) {
        if (Validator.USER.registrationTime(registrationTime)) {
