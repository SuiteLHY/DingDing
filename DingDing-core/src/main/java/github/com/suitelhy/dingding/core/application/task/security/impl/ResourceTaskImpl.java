...
public class ResourceTaskImpl
    ...
    TaskResult<List<ResourceDto>> selectAllResource(int pageIndex, int pageSize) {
        ...
        try {
            @NotNull List<SecurityResource> roleList = securityResourceService.selectAll(pageIndex, pageSize).getContent();
            if (!roleList.isEmpty()) {
                for (SecurityResource each : roleList) {
                    ResourceDto eachRole = ResourceDto.Factory.RESOURCE_DTO.create(each);
    ...
    TaskResult<ResourceDto> selectResourceByCode(@NotNull String resourceCode) {
        ...
        try {
            ...
            final SecurityResource result = securityResourceService.selectResourceByCode(resourceCode);
    ...
    TaskResult<List<ResourceDto>> selectResourceByUsername(@NotNull String username) {
        ...
        final @NotNull List<ResourceDto> resultData = new ArrayList<>(0);
        try {
            @NotNull List<SecurityResource> roleList = userEvent.selectResourceOnUserByUsername(username);
            if (!roleList.isEmpty()) {
                for (SecurityResource each : roleList) {
    ...
    TaskResult<ResourceDto> selectUrlInfoByResourceCode(@NotNull String resourceCode) {
        ...
        try {
            ...
            final SecurityResource result_resource = securityResourceService.selectResourceByCode(resourceCode);
            ...
            if (existedEffectedData) {
                //--- 存在有效（安全）资源的情况
                final @NotNull List<SecurityResourceUrl> result_urlInfo = securityResourceEvent.selectUrlInfoOnResourceByResourceCode(resourceCode);
    ...
    TaskResult<ResourceDto> addResource(@NotNull String resourceCode
        ...
        try {
            ...
            boolean insertResourceSuccessFlag = securityResourceEvent.insertResource(new_resource, securityUserService.selectByUsername(operator_username));
    ...
    TaskResult<Boolean> addResourceToRole(@NotNull ResourceDto existedResource, @NotNull RoleDto existedRole, @NotNull String operator_username) {
        ...
        try {
            ...
            resultData = securityRoleEvent.insertRoleResourceRelationship(securityRoleService.selectRoleByCode(existedRole.getCode())
                    , securityResourceService.selectResourceByCode(existedResource.getCode())
    ...
    TaskResult<Boolean> addUrlToResource(@NotNull ResourceDto existedResource, @NotNull String[] urlInfo, @NotNull String operator_username) {
        ...
        try {
            ...
            resultData = securityResourceEvent.insertResourceUrlRelationship(securityResourceService.selectResourceByCode(existedResource.getCode())
                    , urlInfo
                    , securityUserService.selectByUsername(operator_username));
            if (resultData) {
                //--- 业务操作成功
    ...
    TaskResult<ResourceDto> updateResource(@NotNull @NotNull Map<String, Object> old_resource_data
        ...
        try {
            ...
            boolean updateResourceSuccessFlag = securityResourceService.update(old_resource
                    , new_resource_data
                    , securityUserService.selectByUsername(operator_username));
            // (业务操作是否符合预期)
    ...
    TaskResult<ResourceDto> deleteResource(@NotNull String resourceCode, @NotNull String operator_username) {
        ...
        try {
            ...
            final SecurityResource existedResource = securityResourceService.selectResourceByCode(resourceCode);

            String message = null;
            boolean deleteResourceSuccessFlag;
            if (null != existedResource) {
                deleteResourceSuccessFlag = securityResourceEvent.deleteResource(existedResource, securityUserService.selectByUsername(operator_username));
            } else {
