package github.com.suitelhy.dingding.log.service.client.application.task.impl;
...
public class LogTaskImpl
     ...
     */
    private @NotNull TaskResult<Page<LogDto>> selectAll(int pageCount, int pageSize) {
        @NotNull TaskResult<Page<LogDto>> taskResult;
        @NotNull Page<LogDto> resultData = Page.empty();
        final @NotNull List<LogDto> resultData_content = new ArrayList<>(0);
        try {
            if (pageCount < 1) {
                //-- 非法输入: 页码
            ...
            if (! logPage.getContent().isEmpty()) {
                for (@NotNull Log eachLog : logPage) {
                    resultData_content.add(LogDto.Factory.DEFAULT.create(eachLog));
                }
            }
     ...
     */
    @Override
    public @NotNull TaskResult<Page<LogDto>> selectAll(int pageCount, int pageSize, OAuth2AuthenticationInfo.AbstractUserAuthentication.@NotNull AbstractDetails operator) {
        @NotNull TaskResult<Page<LogDto>> taskResult;
        final @NotNull Page<LogDto> resultData = Page.empty();
        try {
            if (null == operator) {
                //-- 非法输入: 身份认证凭证
    ...
    private @NotNull TaskResult<Log> insert(@NotNull Log param_log) {
        ...
        try {
            ...
            final boolean result = logIdempotentWriteService.insert(param_log);
    ...
    private @NotNull TaskResult<Boolean> delete(@NotNull String id) {
        ...
        try {
            ...
            resultData = logIdempotentWriteService.deleteById(Long.parseLong(id));
